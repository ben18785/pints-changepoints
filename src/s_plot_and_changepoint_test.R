...
data_directory <- as.character(args[1])
data_filename <- as.character(args[2])
output_directory <- as.character(args[3])
output_filename_base <- as.character(args[4])
plot_unimodal <- as.numeric(args[5])
plot_unimodal_logged <- as.numeric(args[6])
plot_unimodal_changepoints <- as.numeric(args[7])
...
# fetch data
test <- f_prep_data(filename)

# fit PELT online changepoint model and add means to data frame ----------
model  <- cpt.mean(test$score, method = "PELT", minseglen = run_length)
...
cpt_alarm_triggered <- if_else(max(cpts) > near_end_threshold, 1, 0)
...
# if unimodal data (most MCMC series plot simple data)
if(plot_unimodal){
  g <-
    test %>% 
  ...
  # add log y scale
  if(plot_unimodal_logged){
    g <- g +
      scale_y_log10()
  }
  
  # add changepoints
  if(plot_unimodal_changepoints){
    g <- g +
...
write(exportJson, paste0(output_filename_base, ".json"))

# output plot
ggsave(paste0(output_filename_base, ".pdf"), g, width = 10, height = 6)
